...
def test_mask_account_card_invalid(account_card, exception):
    with pytest.raises(exception):
        mask_account_card(account_card)


def test_get_date(standart_date):
    assert get_date(standart_date)

#2024-03-11T02:26:18.671407

@pytest.mark.parametrize("incorrect_date, exception",
                         [
...
def test_get_date_incorrect(incorrect_date, exception):
    with pytest.raises(exception):
